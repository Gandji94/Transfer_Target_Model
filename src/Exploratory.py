def run_exploratory():
    ...
    CLEANED_FINAL = ROOT / 'DataSources' /'Processed'/'Cleaned_Final_Stats.csv'
    ...
    #loading the dataset
    df=pd.read_csv(CLEANED_FINAL)
    logging.info('Dataset loaded')
    print('\n')
    print('\n')

    logging.info('-----------------------------Number of NaN Value-----------------------------')
    print('\n')
    null_feat=[]
    ...
    for x in list(df_null.columns):
        if df_null[x][0] > 0.0:
            null_feat.append(x)
    logging.info(f'Number of of columns with Nan values: {len(null_feat)}')
    logging.info(f'Columns with Nan values: {null_feat}')
    print('\n')
    print('\n')
    ...
    logging.info('Dataset splitted in train & test sets')
    print('\n')
    print('\n')


    logging.info('################################## NUMERICA FEATURES #########################################')
    print('\n')
    logging.info('-----------------------------Amount of Numeric and Categorical Features-----------------------------')
    X_train_num=X_train.select_dtypes(exclude='object')
    X_train_cat=X_train.select_dtypes(include='object')
    print('\n')
    logging.info(f'Number of only Numeric Features: {X_train_num.shape[1]}')
    logging.info(f'Number of only Categorical Features: {X_train_cat.shape[1]}')
    print('\n')
    print('\n')

    logging.info('-----------------------------Checking Distribution-----------------------------')
    logging.info("D'Angostin Distribution Test")
    print('\n')
    normal=[]
    non_normal=[]
    ...
    logging.info(f'Normal Distribution: {normal}')
    logging.info(f'Non Normal Distribution: {non_normal}')

    print('\n')
    logging.info("Anderson Distribution Test")
    print('\n')
    normal=[]
    non_normal=[]
    ...
    logging.info('-----------------------------Checking Distribution-----------------------------')
    logging.info('Outlier detection IQR')
    outlier_list=[]
    for x in list(X_train_num.columns):
    ...
    logging.info((len(outlier_list)/len(list(X_train_num.columns)))*100)
    logging.info(outlier_list)
    print('\n')
    logging.info('Outlier detection Robust Z-Score')
    outlier_z=[]
    for x in list(X_train_num.columns):
        rbzs=abs(RobustZ_Score(X_train_num[x]))
        if np.any(rbzs > 3):
            outlier_z.append(f'{x}-{(round(sum(rbzs > 3)/len(X_train_num),2))*100}%')
    logging.info((len(outlier_z)/len(list(X_train_num.columns)))*100)
    logging.info(outlier_z)

    print('\n')
    print('\n')

    logging.info('Boxplots, QQ-Plots & Histograms')
    logging.info('Look it up in the notebook "Exploratory_Transfer_Target_Model".')

    print('\n')
    print('\n')

    logging.info('-----------------------------Corrleation & Multicollinarity-----------------------------')
    ...
    logging.info(f"Number of affected Columns that causes Multicollinarity: {len(to_drop)}")
    logging.info(f"Percentage of affected Features: {round((len(to_drop)/len(list(X_train_num.columns)))*100,2)}")
    print('\n')
    logging.info('VIF')
    ...
    logging.info(f"Number of affected Columns that causes Multicollinarity: {len(VIF)}")
    logging.info(f"Percentage of affected Features: {round((len(VIF)/len(X_train_num.columns))*100,2)}")
    logging.info(VIF)
    ...
    logging.info('################################## NUMERICA FEATURES #########################################')
    logging.info('y Class appreances')
    ...
    logging.info('Linearity Check for Encoded Features')
    logging.info('Look it up in the notebook "Exploratory_Transfer_Target_Model".')
    print('\n')


    logging.info('25th, median, 75th of encoded features')
    for ec in ['Nation','Club_Left','League_Left']:
        logging.info(f'{ec}')
        logging.info(df_median_encoded.groupby('y_train')[ec].agg(
    ...
    print('\n')
    logging.info('Point Biserial Correlation')
    ...
    logging.info(point_corr_df[point_corr_df['P-Value']<0.05])
    print('\n')
    logging.info('Linearity CHeck of all X features based on y classes')
    ...
    for x in x_feat:
        if x == 'League_Joined':
            ...
            print('\n')
            print('\n')
    logging.info('The graphics are avaiable on "Exploratory_Transfer_Target_Model"')
    print('\n')
    print('\n')

    logging.info('################################## ANOVA Test #########################################')
    logging.info('Equal Variance Assumption')
    x_feat=list(final_adjust_df.columns)
    ...
    logging.info(f'Equal Variance: {equal_variance}')
    logging.info(f'Apperances of Equal Variance: {len(equal_variance)}')
    logging.info(f'Unequal Variance: {unequal_variance}')
    logging.info(f'Apperances of unequal Variance: {len(unequal_variance)}')
    print('\n')

    logging.info('Normality Assumption')
    x_feat=list(final_adjust_df.columns)
    for d in ['Player','Squad','League','Pos_MF','Pos_FW','Pos_GK','Pos_DF','Age_Early Twenties','Age_Late Twenties','Age_Teenager','Age_Early Thirties','Age_Late Thirties','League_Joined']:
        x_feat.remove(d)
    ...
    for c in list(final_adjust_df['League_Joined'].unique()):
        for x in x_feat:
            ...
            if p_value < 0.05:
                ...
                normal.append(f'{c}-{x}')
    logging.info(f'Normal Distrbution: {normal}')
    logging.info(f'Appearances of Normal Distrbution: {len(normal)}')
    logging.info(f'Non-Normal Distrbution: {non_normal}')
    logging.info(f'Appearances of Non-Normal Distrbution: {len(non_normal)}')

    print('\n')
    logging.info('Kruskal-Wallis (ANOVA) & Dunn Test (Ado Hoc/Pairwise Test)')
    ...
    logging.info(f'Number of unique statistically significant relationshipt between Features and Leagues Joined: {len(pairwise_results["feature"].unique())}')
    logging.info(pairwise_results["feature"].unique())
    logging.info(pairwise_results[pairwise_results['p-unc']<0.05])
    print('\n')
    print('\n')

    logging.info('################################## Chi-Square Test #########################################')
    ...
    # contingency table
        ...
        logging.info(f"{col} | Asymptotic p: {p_asymp:.4g} | Permutation p: {p_perm:.4g} | Cramér's V: {cramers_v:.3f} | % expected<5: {prop_small:.2%}")
