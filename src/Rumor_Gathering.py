def webscrapper_rumors():
    ...
    DATA_RUMORS = ROOT / "DataSources" / "Processed" / "Rumor_Overview.csv"
    DATA_STATS_W_RUMORS = ROOT / "DataSources" / "Processed" / "Cleaned_Final_Stats_w_Rumors.csv"

    df=pd.read_csv(DATA_CLEANED_FINAL_STATS)
    ...
    rumor_df.to_csv(DATA_RUMORS,index=False)
    ...
    df_rumor=pd.read_csv(DATA_RUMORS).drop(['probability_raw','page','probability_pct','club_id'],axis=1)
    ...
    df_final.to_csv(DATA_STATS_W_RUMORS,index=False)
